...
public class IRAAIAgentPage {
	...
	public static final String BOT_TEXT = "//div[@aria-label='You sent, Text." + GREETINGS_MSG
	...
	// public static final String MALE_INPUT =
	// "//div[@data-ele-name='Gender']/following-sibling::*/label[*[text()='Male']]";
	// public static final String FEMALE_INPUT =
	// "///div[@data-ele-name='Gender']/following-sibling::*/label[*[text()='Female']]";
	...
	public static final String RATING_HEADER_ERROR = "//div[normalize-space(text())='Rate the experience']";
	...
	// RENEW POLICY
	public static final String RENEW_POLICY_MSG = "Renew Motor Policy";
	...
	public static final String REGISTRATION_NUMBER_BOT_QUES = "//div[contains(@aria-label,'Can you help me with your vehicle registration number?')]";
	...
	public static final String VERIFY_POLICY_DETAILS_TABLE = "//*[normalize-space(text())='Thanks for entering all the details. The details you have entered are:']/table/tbody";
	public static final String RENEW_SUCCESSFUL = "//p[contains(text(), 'Your policy will be renewed..Have a great day')]";
	...
	public boolean renewPolicy(String policyNo, String registrationNo, String mobileNo, String email) {
		typeMsg(RENEW_POLICY_MSG);
		getWebDriverWait().until(ExpectedConditions.presenceOfElementLocated(By.xpath(POLICY_NUMBER_BOT_QUES)));
		typeMsg(policyNo);
		getWebDriverWait().until(ExpectedConditions.presenceOfElementLocated(By.xpath(REGISTRATION_NUMBER_BOT_QUES)));
		typeMsg(registrationNo);
		getWebDriverWait().until(ExpectedConditions.presenceOfElementLocated(By.xpath(MOBILE_NUMBER_BOT_QUES)));
		typeMsg(mobileNo);
		getWebDriverWait().until(ExpectedConditions.presenceOfElementLocated(By.xpath(EMAIL_ID_BOT_QUES)));
		typeMsg(email);
		getWebDriverWait().until(ExpectedConditions.presenceOfElementLocated(By.xpath(VERIFY_RENEW_POLICY_DETAILS)));

		return getWebElement(VERIFY_RENEW_POLICY_DETAILS).isDisplayed();

	}

	public boolean renewData(LinkedHashMap<String, String> hp) {
		boolean checkValue = false;
		for (String title : hp.keySet()) {
			checkValue = getWebElement(
					"//*[text()[contains(.,'Thanks for entering all the details.')]]/table//tr/td[contains(text(),'"
							+ title + "')]/following-sibling::td").getText().equals(hp.get(title));
			if (checkValue == false)
				getWebElement("//a[contains(@href, 'No')]").click();
			break;
		}
		getWebElement("//a[contains(@href, 'Yes')]").click();
		getWebDriverWait().until(ExpectedConditions.presenceOfElementLocated(By.xpath(RENEW_SUCCESSFUL)));
		return getWebElement(RENEW_SUCCESSFUL).isDisplayed();
	}
	...
	public IRAAIAgentPage reset() {
		getWebDriverWait().until(ExpectedConditions.elementToBeClickable(By.xpath(ENTER_TEXT)));
		getWebElement(ENTER_TEXT).sendKeys("reset");
		getWebElement(SEND_TEXT).click();
		return this;

	}

	public String fillAndSubmitform(String msg, String name, String address, String gender, String freq,
			String rating) {
		getDriver().switchTo().frame(AVAAMO_FRAME);
		typeMsg(msg);
		...
		((JavascriptExecutor) getDriver()).executeScript("arguments[0].scrollIntoView(true);",
				getWebElement("//span[text()='" + gender + "']"));
		getWebElement("//span[text()='" + gender + "']").click();
		((JavascriptExecutor) getDriver()).executeScript("arguments[0].scrollIntoView(true);",
				getWebElement(SELECT_DROPDOWN));
		getWebElement(SELECT_DROPDOWN).click();
		getWebElement("//li[text()='" + freq + "']").click();
		getWebElement("//div[@data-ele-name='Rate the experience']/following-sibling::div//label[@aria-label='" + rating
				+ "']").click();
	...
	public boolean clickGoogleAndCall(String msg) {
		...
		// as google popup doesnt launch any title or content body, verifying using
		// close button
